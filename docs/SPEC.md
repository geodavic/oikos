...
| completed_by | INTEGER | FK → Users, ON DELETE SET NULL, nullable (migration v152) — the member who actually did the task, asked for on every completion. This, not `assigned_to`, decides who the `points` are credited to; see below. Cleared whenever the task leaves `done` |
| completed_at | TEXT | ISO timestamp, nullable (migration v152) — when it was ticked off. Cleared together with `completed_by` |
| points | INTEGER | NOT NULL DEFAULT 0 — reward points credited on completion to whoever `completed_by` names, or, when it is NULL, to the assigned members (Rewards module, migration v69; attribution changed in v152) |
...
**Completion attribution (migration v152):** completing a task asks **who did it**, on every surface that can tick one off — the list checkbox, the swipe, the detail view, the edit form's status dropdown, both kanban gestures, the dashboard widget, subtask checkboxes, and bulk "mark done" (which asks once for the whole selection). The answer is stored in `completed_by` and is who the reward points go to, replacing the previous rule that paid the **assignees**. That rule was wrong in the case the household hits daily: a parent doing a child's chore on the shared wall tablet credited the child. If the named person does not take part in the points system, **nobody** is credited — falling back to the assignee would restore exactly the bug. The field is **optional on the API**, because the CalDAV inbound sync, API tokens and older clients cannot answer the question; without it the old assignee-based crediting still applies. In a household of one the question is not asked, for the same reason the visibility selector is not shown. The recurrence follow-up is created unattributed, and reopening a task clears the attribution along with its ledger entry. Housekeeping decay chores carry the same question in `housekeeping_decay_tasks.last_completed_by`, where — having neither assignees nor points — it is the only record that anyone in particular did the work.
